...
extern "C" __global__ void __closesthit__radiance() {
    ...
    const float3 lightV1 = make_float3(0.46f, 0.0, 0.0f);
    const float3 lightV2 = make_float3(0.0f, 0.0, 0.46f);
    ...
    if (nDl > 0.0f && LnDl > 0.0f)
    {
        ...
        optixTrace(optixLaunchParams.traversable,
            ...
            L,
            0.001f,         // tmin
            Ldist + 0.01f,//1e20f, //Ldist - 0.01f,  // tmax
            0.0f,                    // rayTime
...

    {
        ...
        float spec_frac = pow(clamp(dot(normalize(optixGetWorldRayDirection()), normalize(reflect(L, nn))), 0.0, 1.0), sbtData.shininess);
    ...
    prd.radiance += weight * shadow_prd.shadowAtt * optixLaunchParams.global->lightScale / continue_probability;
}
